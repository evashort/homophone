...
import Repronounce exposing (Cache, CostData, Status(..))
...
cacheExample :
  List (List (List String)) -> List (String, Float) ->
    List (String, List (String, Float)) -> List (String, Float) -> List Status
...
asdf :
  CostData -> List (List String) -> (List Status, Cache) ->
    (List Status, Cache)
asdf data sentence (statuses, cache) =
  let result = Repronounce.repronounce data cache sentence 100 in
    (result.status :: statuses, result.cache)


respellExample :
  List (List String) -> List (String, Float) ->
    List (String, List (String, Float)) -> List (String, Float) -> Status
  ...
  in
    case (maybeDeletionCosts, maybeSubCosts, maybeWordCosts) of
      (Just deletionCosts, Just subCosts, Just wordCosts) ->
        ...
        in let
          result =
            Repronounce.repronounce data Repronounce.emptyCache sentence 100
        in
          result.status
